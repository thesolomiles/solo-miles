...
const LEVELS = [
  { id: 'N5', note: 'Beginner' },
  { id: 'N4', note: 'Elementary' },
  { id: 'N3', note: 'Intermediate' },
  { id: 'N2', note: 'Upper' },
  { id: 'N1', note: 'Advanced' },
] as const
          ...
          {LEVELS.map((l) => {
            ...
            return (
              ...
              >
                ...
                <span className="opacity-60">{l.note}</span>
              </button>
            )
          })}
        </div>
          ...
          {DECKS.map((deck) =>
            ready ? (
              <button
                key={deck.slug}
                type="button"
                onClick={() => router.push(`/projects/japanese-flashcards/${deck.slug}`)}
              ...
              >
                <span className="flex w-full items-baseline justify-between gap-3">
                  <span className="font-display text-2xl leading-none font-extrabold tracking-[-0.02em]">{deck.glyph}</span>
