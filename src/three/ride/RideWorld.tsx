 ...
 * grain speckle and a few faint cracks. Scrolled down the road each frame so the
 * surface reads as moving.
 */
function makeTarmacTexture(): THREE.CanvasTexture {
  const S = 256
  const c = document.createElement('canvas')
  c.width = c.height = S
  const ctx = c.getContext('2d')!
  const hex = (n: number) => '#' + n.toString(16).padStart(6, '0')
  ctx.fillStyle = hex(RIDE_COLORS.road)
  ctx.fillRect(0, 0, S, S)
  ...
  for (let i = 0; i < 26; i++) {
    ...
    g.addColorStop(0, rand() < 0.55 ? hex(RIDE_COLORS.roadDark) : hex(RIDE_COLORS.roadLight))
  ...
  for (let i = 0; i < 2600; i++) {
    ...
    ctx.fillStyle = rand() < 0.5 ? hex(RIDE_COLORS.roadDark) : hex(RIDE_COLORS.roadLight)
  ...
  ctx.strokeStyle = hex(RIDE_COLORS.roadDark)
  ctx.lineWidth = 1
  for (let i = 0; i < 7; i++) {
    let x = rand() * S
    let y = rand() * S
